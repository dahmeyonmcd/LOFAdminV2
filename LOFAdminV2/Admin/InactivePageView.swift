...
class InactivePageView: UICollectionViewCell, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    fileprivate let categoriesArray = ["Inactive"]
                ...
                if ((response.result.value) != nil) {
                    ...
                    if let innerJson = jsondata["success"].array {
                        for results in innerJson {
                            ...
                            if allMembers.count == innerJson.count {
                                let sorted = allMembers.filter({$0.active == false})
