...
class LineChart: UIView {
    ...
    let lineGap: CGFloat = 55.0
    
    /// preseved space at top of the chart was 40.0
    ...
    private func setupView() {
        ...
        gradientLayer.colors = [#colorLiteral(red: 1, green: 1, blue: 1, alpha: 0.06511455141).cgColor, UIColor.clear.cgColor]
        scrollView.layer.addSublayer(gradientLayer)
        self.layer.addSublayer(gridLayer)
        self.addSubview(scrollView)
        self.backgroundColor = #colorLiteral(red: 0.950980189, green: 0.950980189, blue: 0.950980189, alpha: 1)
    }
    ...
    private func drawCurvedChart() {
        ...
        if let path = CurveAlgorithm.shared.createCurvedPath(dataPoints) {
            let lineLayer = CAShapeLayer()
            lineLayer.path = path.cgPath
//            lineLayer.strokeColor = #colorLiteral(red: 0.3422607481, green: 0.7863087058, blue: 0.9452798963, alpha: 1)
...
//            gradient.frame = lineLayer.path as! CGRect
//            gradient.mask = lineLayer
//            gradient.colors = [#colorLiteral(red: 0.3422607481, green: 0.7863087058, blue: 0.9452798963, alpha: 0.5).cgColor, #colorLiteral(red: 0.1960784314, green: 0.9171321988, blue: 0.7997155786, alpha: 0.5).cgColor]
//
//
//            let shapeMask = CAShapeLayer()
//            shapeMask.path = path.cgPath
//            gradient.mask = shapeMask
//            lineLayer.addSublayer(gradient)
    ...
    private func drawHorizontalLines() {
        ...
        if dataEntries.count < 5 && dataEntries.count > 0 {
            gridValues = [0, 1]
        } else if dataEntries.count >= 5 {
        ...
        if let gridValues = gridValues {
            for value in gridValues {
                ...
                let lineLayer = CAShapeLayer()
                lineLayer.path = path.cgPath
                lineLayer.fillColor = #colorLiteral(red: 0.542514205, green: 0.6135223508, blue: 0.7381866574, alpha: 0.6366348735).cgColor
                lineLayer.strokeColor = #colorLiteral(red: 0.542514205, green: 0.6135223508, blue: 0.7381866574, alpha: 0.6391152224).cgColor
    ...
    private func drawDots() {
        ...
        if let dataPoints = dataPoints {
            for dataPoint in dataPoints {
                ...
                dotLayer.dotInnerColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
                dotLayer.innerRadius = innerRadius
                dotLayer.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
                dotLayer.cornerRadius = outerRadius / 2
                dotLayer.frame = CGRect(x: xValue, y: yValue, width: outerRadius, height: outerRadius)
                dotLayers.append(dotLayer)

                dotLayer.borderWidth = 10
